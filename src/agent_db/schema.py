    ...
    if git_root is not None:
        for directory in paths_to_cwd(cwd, git_root):
            ...
            if claude_md.is_file():
                ...
                sources.extend(_resolve_at_imports(claude_md, Agent.CLAUDE, Scope.PROJECT, "project"))
            ...
            if dot_claude_md.is_file():
                sources.append(InstructionSource(
                    ...
                    path_globs=None,
                    description=f"Project instructions at {dot_claude_md.relative_to(git_root)}",
                ))
                sources.extend(_resolve_at_imports(dot_claude_md, Agent.CLAUDE, Scope.PROJECT, "project"))
            ...
            if project_rules_dir.is_dir():
                for rule_file in sorted(project_rules_dir.glob("*.md")):
                    ...
                    sources.append(InstructionSource(
                        ...
                        source_type=SourceType.RULES,
                        scope=Scope.PROJECT,
                        load_timing=LoadTiming.ON_READ_MATCH if is_path_scoped else LoadTiming.STARTUP,
                        output_path=rule_file,
                        layer_name="project",
                        source_path=rule_file,
        ...
        for directory in paths_to_cwd(cwd, git_root):
            ...
            if project_override.is_file():
                sources.append(InstructionSource(
                    ...
                    path_globs=None,
                    requires_trust=True,
                    description=f"Project AGENTS.override.md at {project_override.relative_to(git_root)}",
                ))
...
def _resolve_at_imports(
    ...
    for match in AT_IMPORT.finditer(content):
        ...
        if not candidate.is_file():
            continue
        imported.append(InstructionSource(
            agent=agent,
            source_type=SourceType.RULES,
            scope=scope,
            load_timing=LoadTiming.STARTUP,
        ...
        for directory in paths_to_cwd(cwd, git_root):
            ...
            if project_settings.is_file():
                sources.append(SettingsSource(
                    ...
                    requires_trust=True,
                    precedence_order=2,
                    description=f"Project settings.json at {project_settings.relative_to(git_root)}",
                ))
            ...
            if local_settings.is_file():
                sources.append(SettingsSource(
                    ...
                    source_path=local_settings,
                    precedence_order=3,
                    description=f"Local settings.json at {local_settings.relative_to(git_root)}",
                ))
...
def _extract_all_claude_rules(settings: dict[str, Any], settings_path: Path) -> tuple[RulesSource, ...]:
...
def _load_codex_rules(
    ...
    if user_rules_dir.is_dir():
        for rules_file in sorted(user_rules_dir.glob("*.rules")):
            sources.extend(_parse_codex_rules_file(
                rules_file,
                scope=Scope.USER,
                git_root=git_root,
            ))
