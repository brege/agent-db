...
def write_global(source: AgentSource, codex_home: Path) -> tuple[list[Path], list[str]]:
    ...
    rendered = render_instruction_file(agents_md.name, assembly.sections, assembly.settings)
    if files.write_text(agents_md, rendered):
        written.append(agents_md)

    config = home / "config.toml"
    if write_config(config, render_config(assembly.settings)):
    ...
    written.extend(write_agents(assembly.agents, home / "agents"))
...
def write_config(path: Path, generated: str) -> bool:
    ...
    if existing == layered:
        return False
    return files.write_text(path, layered)


def layer_config(existing: str, generated: str) -> str:
    generated = generated.strip()
    if not generated:
        return remove_managed_config(existing)
